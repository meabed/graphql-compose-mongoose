# graphql-compose-mongoose-extended
...
```bash
npm install graphql graphql-compose mongoose graphql-compose-mongoose-extended --save
```
...
Viktor Kjartansson created a quite solid intro for `graphql-compose-mongoose-extended` in comparison with `graphql-tools`:
...
import { composeMongoose } from 'graphql-compose-mongoose-extended';
...
type FilterHelperArgsOpts = {
   ...
   * By default, for performance reason, `graphql-compose-mongoose-extended` generates operators
